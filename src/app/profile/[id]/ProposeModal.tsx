...
export function ProposeModal({
  ...
  function handleSend() {
    startTransition(async () => {
      const result = await sendInvite(targetProfileId, message || null)
      if (result.error) {
        alert(result.error)
        return
      }
        ...
        >
          <div className="w-full max-w-md rounded-2xl border border-neutral-800 bg-neutral-950 p-6 space-y-5 shadow-2xl">
            <div className="space-y-1">
              <h2 className="text-base font-semibold">Proposta a {targetName}</h2>
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                placeholder={`Ciao ${targetName.split(' ')[0]}, mi piacerebbe collaborare...`}
                rows={3}
            ...
            <p className="text-xs text-neutral-700 text-center">
              Il destinatario riceverà una notifica e potrà accettare o rifiutare.
            </p>
          </div>
        </div>
      )}
